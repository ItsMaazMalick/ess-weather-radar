...
// Loaded via require() of a .json file rather than fs.readFileSync(a computed
// path), deliberately: Next.js's server bundler rewrites __dirname to a
// synthetic placeholder for traced serverless modules, so a path built with
// path.join(__dirname, ...) resolves to nothing at runtime on Vercel even
// though it works locally. require()'ing a .json module is instead resolved
// through the bundler's own module graph, which both Node and Next/webpack
// handle correctly regardless of __dirname.
...
function load() {
  ...
  try {
    const geojson = require('../data/pakistan_basins_lev06.json');
